...
- **实时对战主流程**：注册登录、创建/加入房间、准备开局、发牌、出牌、PASS、结算、排行榜、断线重连。
- **分布式部署**：支持 Nginx + 多 Spring Boot 实例 + Redis Session/房间状态 + RabbitMQ STOMP Relay，适合演示水平扩展。
- **充值/VIP 系统**：支持套餐下单、模拟支付、订单状态流转、VIP/SVIP 授权与到期时间计算。
- **账号安全**：支持绑定手机号、短信验证码、人机验证、手机号找回密码、登录后修改密码。
- **内容安全**：局内聊天、私信、昵称、房间简介复用敏感词检测；支持脱敏、拦截、频控、重复消息检测、自动禁言。
- **后台管理**：`/admin` 仅 ADMIN 可访问，支持敏感词库维护、违规审计日志、禁言查看/解除、用户检索、账号封禁/解封。
- **账号封禁**：后台可按用户名/昵称搜索账号，设置永久/小时/天封禁；登录、HTTP 请求和 WebSocket 消息都会识别封禁状态，登录失败会展示封禁原因和到期时间。
- **可观测性**：Actuator + Prometheus + Grafana，提供实例状态、JVM、HTTP、业务房间数等指标。
- **测试覆盖**：已补充游戏规则、充值、内容安全、手机号验证码、人机验证、账号封禁等核心单元/集成测试。
...
后台入口：`/admin`，需要账号具备 `ADMIN` 角色。

后台当前提供：

- 用户概览：总用户数、管理员数、VIP/SVIP 数。
- 用户管理：按用户名或昵称检索用户；查看角色、积分、战绩、封禁状态、封禁原因和到期时间。
- 账号封禁：支持永久封禁、按小时封禁、按天封禁；禁止封禁自己和其他管理员账号。
- 登录封禁提示：被封禁账号登录时提示封禁原因和封禁时长，而不是泛化为“用户名或密码错误”。
- 内容安全：维护脱敏词/拒绝词；查看违规审计日志；查看并解除仍在生效的自动禁言。

---

## 账号安全与验证码

- 绑定手机号前需要先通过图形/滑块式人机验证，再发送 6 位短信验证码。
- 验证码有有效期和发送冷却时间，避免用户频繁请求。
- 手机号可用于找回密码；登录后也可通过手机号验证码修改密码。
- 当前短信发送默认支持调试模式，开发环境会返回/打印验证码，生产环境可替换为真实短信网关。

---

## 内容安全

内容安全服务统一复用于局内聊天、私信、昵称和房间简介。

处理策略包括：

- 脱敏词：替换为 `*` 后继续发送。
- 拒绝词：阻止提交并写入审计日志。
- 频控/重复消息：限制刷屏并可触发自动禁言。
- 管理后台：支持维护敏感词库、查看审计日志、解除禁言。
...
| `RABBITMQ_USERNAME` / `RABBITMQ_PASSWORD` | RabbitMQ 账号 / 密码 | `guest` / `guest` |
