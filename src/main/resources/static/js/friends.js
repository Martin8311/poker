...
(function () {
    ...
    function loadFriends() {
        ...
        fetch('/friends')
            ...
            .then(list => {
                ...
                box.innerHTML = list.map(f => row(f.iconUrl, f.nickname, f.online ? '在线' : '离线',
                    `<button class="friend-btn del" data-remove="${esc(f.username)}">删除</button>`, f.online)).join('');
                bindRowActions(box);
            })
    ...
    function refreshBadge() {
        fetch('/friends/requests').then(r => r.json())
            .then(list => updateBadge(list.length)).catch(() => {});
    }
    ...
    // ---------------- 导出 + 自动初始化 ----------------
    window.FriendsUI = {
        open, close, sendRequest, onNotification, refreshBadge, connectStandalone
    };
